...
param(     
	...
	[alias("MasterPage")]$Layout = "~/Views/Shared/_LayoutInk.cshtml",
...
if (!$NoChildItems) {
	if ($Repository) {
		...
		$dbContextScaffolderResult = Scaffold InkDbContext -ModelType $foundModelType.FullName -DbContextType $DbContextType -DbContextPropName $DbContextPropName -Area $Area -Project $Project -CodeLanguage $CodeLanguage
...
if (!$NoChildItems) {
	...
	if ($ViewScaffolder) {
		Scaffold InkViews -ViewScaffolder $ViewScaffolder -Controller $controllerNameWithoutSuffix -ModelType $foundModelType.FullName -Area $Area -Layout $Layout -SectionNames $SectionNames -PrimarySectionName $PrimarySectionName -ReferenceScriptLibraries:$ReferenceScriptLibraries -Project $Project -CodeLanguage $CodeLanguage -Force:$overwriteFilesExceptController
	}
}
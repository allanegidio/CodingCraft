...
if ($foundDbContextType) {
	...
	if (!$isDbContext) {
		...
		$existingMembers = [T4Scaffolding.Core.EnvDTE.EnvDTEExtensions]::PowerShellInvoke([T4Scaffolding.Core.EnvDTE.EnvDTEExtensions].GetMethod("VisibleMembers"), $null, $foundDbContextType)
		$hasExistingPropertyForModel = $existingMembers | ?{ ($_.Name -eq $DbContextPropName) -and ($_.Kind -eq [EnvDTE.vsCMElement]::vsCMElementProperty) }
		if ($hasExistingPropertyForModel) {
			Write-Warning "$($foundDbContextType.Name) already contains a '$DbContextPropName' property."
		} else {
			if(!$DbContextPropName) {
				...
				Add-ClassMemberViaTemplate -Name $propertyName -CodeClass $foundDbContextType -Template DbContextEntityMember -Model @{
					EntityType = $foundModelType;
					EntityTypeNamePluralized = $DbContextPropName;
				} -SuccessMessage "Added '$DbContextPropName' to database context '$($foundDbContextType.FullName)'" -TemplateFolders $TemplateFolders -Project $Project -CodeLanguage $CodeLanguage
			}
		}		
	}
}
